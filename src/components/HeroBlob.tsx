...
function Blob({ isMobile }: { isMobile: boolean }) {
  ...
  // Create a custom geometry that we can morph. Use lower detail on mobile.
  const geometry = useMemo(() => new THREE.IcosahedronGeometry(2, isMobile ? 16 : 64), [isMobile]);
  ...
  useFrame((state) => {
    ...
    // Only do manual vertex morphing on desktop for the complex transmission material
    if (!isMobile) {
